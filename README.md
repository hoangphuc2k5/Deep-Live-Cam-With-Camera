# Deep-Live-Cam 2.1 (GitHub Edition)

Dự án này là một công cụ thực hiện **face swap** (đổi mặt) thời gian thực trên webcam hoặc xử lý video/hình ảnh chỉ với một tấm ảnh nguồn. Phiên bản này bao gồm cả giao diện máy tính (Desktop GUI) và giao diện web (Web API).

---

## 🚀 Tính năng chính

- **Real-time Face Swap**: Đổi mặt trực tiếp trên Webcam với tốc độ cao.
- **Video/Image Processing**: Xử lý đổi mặt cho các file video và ảnh có sẵn.
- **Mouth Mask**: Giữ nguyên vùng miệng gốc để có cử động tự nhiên hơn.
- **Face Mapping**: Chỉ định đổi mặt cho từng người cụ thể nếu trong khung hình có nhiều người.
- **Enhancer**: Tích hợp các bộ lọc làm nét mặt (GFPGAN, GPEN).
- **Web Interface**: Cung cấp giao diện Web qua FastAPI (hỗ trợ tài khoản và quản lý tác vụ).
- **Đa ngôn ngữ**: Hỗ trợ nhiều ngôn ngữ bao gồm Tiếng Việt, Tiếng Anh, Tiếng Trung, v.v.
...
- **Python**: 3.9 hoặc mới hơn (Khuyên dùng 3.10).
- **GPU (Khuyên dùng)**: NVIDIA (hỗ trợ CUDA) để đạt tốc độ xử lý tốt nhất.
- **FFmpeg**: Cần thiết để xử lý video và âm thanh.
...
4. **Tải Models**:
   Tải các file model `.onnx` cần thiết và đặt vào thư mục `models/` (xem file `models/instructions.txt` để biết chi tiết).

---

## 🎮 Cách sử dụng

### 1. Giao diện Desktop (GUI)
Để mở giao diện phần mềm truyền thống:
```powershell
python run.py
```
*Gợi ý:* Nếu bạn có GPU NVIDIA, hãy dùng: `.\main_cuda.bat` hoặc `python run.py --execution-provider cuda`.

### 2. Giao diện Web (API & Browser)
Dự án tích hợp sẵn một giao diện web tiện lợi:
```powershell
python main.py
```
Sau đó truy cập: `http://localhost:8000` trên trình duyệt.

### 3. Chạy qua Command Line (Headless)
Ví dụ xử lý video:
```powershell
python run.py -s "face.jpg" -t "video.mp4" -o "output.mp4" --frame-processor face_swapper face_enhancer
```
...
Công cụ này được tạo ra vì mục đích học thuật và sáng tạo. Người dùng phải chịu hoàn toàn trách nhiệm về nội dung tạo ra.
- **Không sử dụng** để tạo nội dung nhạy cảm, đồi trụy hoặc vi phạm pháp luật.
- **Không sử dụng** khuôn mặt của người khác khi chưa có sự đồng ý.
- Luôn ghi rõ nội dung là deepfake khi chia sẻ công khai.
