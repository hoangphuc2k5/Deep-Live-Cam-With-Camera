...
def _execute_ffmpeg(commands: List[str]) -> tuple[bool, str]:
    try:
        ...
        msg = f"ffmpeg execution failed: {error}"
        print(msg)
        return False, msg


def run_ffmpeg(args: List[str]) -> bool:
    """Run ffmpeg with hardware acceleration and retry with software decode on failure."""
    base_commands = [
        "ffmpeg",
        "-hide_banner",
    ...
    hwaccel_commands = [
        *base_commands,
        ...
        "-hwaccel_output_format", "auto",  # Some FFmpeg builds may reject this at runtime
        *args,
    ]
    success, output = _execute_ffmpeg(hwaccel_commands)
    if success:
        return True

    # Retry without hwaccel flags for systems where D3D/driver integration fails.
    lowered = output.lower()
    should_retry_software = any(
        marker in lowered
        for marker in [
            "unrecognised hwaccel output format",
    ...
    if should_retry_software:
        print("FFmpeg hardware acceleration failed, retrying with software decode...")
        software_commands = [*base_commands, *args]
        success, _ = _execute_ffmpeg(software_commands)
        return success
...
def extract_frames(target_path: str) -> bool:
    ...
    if not frame_paths:
        print(
            f"Frame extraction succeeded but produced no PNG frames in: {temp_directory_path}"
        )
        return False
    return True
...
def create_video(target_path: str, fps: float = 30.0) -> bool:
    ...
    if not frame_paths:
        print(f"No temporary PNG frames found for encoding in: {temp_directory_path}")
        print(f"Expected ffmpeg frame pattern: {frame_pattern}")
        return False
...
def restore_audio(target_path: str, output_path: str) -> None:
    ...
    # Try to copy the original audio stream directly into the encoded video.
    copy_args = [
        "-i",
        temp_output_path,
        "-i",
    ...
    if run_ffmpeg(copy_args) and os.path.isfile(output_path):
        return

    # Fallback: re-encode audio to AAC if direct copy is not supported by the container/codecs.
    fallback_args = [
        "-i",
        temp_output_path,
        "-i",
    ...
    if run_ffmpeg(fallback_args) and os.path.isfile(output_path):
        return
...
def has_image_extension(image_path: str) -> bool:
    return image_path.lower().endswith(("png", "jpg", "jpeg", "webp", "bmp", "gif"))
