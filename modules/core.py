...
def start() -> None:
    ...
    if not modules.globals.map_faces:
        ...
        extracted = extract_frames(modules.globals.target_path)
        if not extracted:
            update_status('Frame extraction failed. Input may be invalid or ffmpeg could not decode it.')
    ...
    if total_frames == 0:
        update_status('No extracted frames were found. Stopping before video encoding.')
        clean_temp(modules.globals.target_path)
        return
